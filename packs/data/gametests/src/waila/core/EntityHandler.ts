...
export class EntityHandler {
	...
	static createRenderData(
		...
		const extraInfo = isPlayer ? [] : EntityHandler.buildExtraInfo(entity);
	...
	// R4.8: 实体详细信息（借鉴 Block & Entity Details，蹲下时显示）
	private static buildExtraInfo(entity: Entity): RawMessage[][] {
		...
		try {
			...
			if (baby) {
				// 注意：ageable.duration 是成长总时长（固定值），不是剩余时间；
				// Bedrock 脚本 API 读不到当前年龄 → 无法计算剩余/百分比，只显示"幼年"
				lines.push([{ text: "§7幼年" }]);
			}
		} catch {
			/* 忽略 */
		...
		try {
			if (entity.getComponent("minecraft:is_tamed")) {
				lines.push([{ text: "§7已驯服: §a是" }]);
			} else {
				const tameable = entity.getComponent("minecraft:tameable") as
					| { probability?: number }
					| undefined;
				if (tameable && typeof tameable.probability === "number" && tameable.probability > 0) {
					lines.push([{ text: `§7驯服几率: §a${Math.round(tameable.probability * 100)}%` }]);
				}
			}
		} catch {
			/* 忽略 */
		...
		// ── 坐下（狼/猫等，非标准组件字符串访问）──
		try {
			if (entity.getComponent("minecraft:is_sitting")) {
				lines.push([{ text: "§7坐下: §e是" }]);
			}
		} catch {
			/* 忽略 */
		}
		...
		// ── 充能苦力怕 ──
		try {
			if (entity.getComponent("minecraft:is_charged")) {
				lines.push([{ text: "§7充能: §b是" }]);
			}
		} catch {
			/* 忽略 */
		}
		...
		// ── 僵尸村民转化中 ──
		try {
			const transforming = entity.getComponent("minecraft:is_transforming");
			const hasTransTag = entity.getTags().some((t) =>
				/transforming|transformation/i.test(t),
			);
			if (transforming || hasTransTag) {
				lines.push([{ text: "§7转化中 §e(僵尸村民)" }]);
			}
		} catch {
			/* 忽略 */
		}
		...
		// ── 坐骑速度 ──
		try {
			if (EntityHandler.isRideable(typeId)) {
				const mv = entity.getComponent("minecraft:movement") as
					| { currentValue?: number }
					| undefined;
				if (mv && typeof mv.currentValue === "number") {
					const bps = (mv.currentValue * 43.17).toFixed(1);
					lines.push([{ text: `§7速度: §b${bps} §7方块/秒` }]);
				}
			}
		} catch {
			/* 忽略 */
		...
		// ── 效果 ──
		try {
			const fx = entity.getEffects();
			if (fx && fx.length > 0) {
				const names = fx.slice(0, 3).map((e) => {
					const id = e.typeId.replace("minecraft:", "").replace(/_/g, " ");
					return id;
				});
				const more = fx.length > 3 ? ` §8+${fx.length - 3}` : "";
				lines.push([{ text: `§7效果: §d${names.join("§7, §d")}${more}` }]);
			}
		} catch {
			/* 忽略 */
		}
	...
	private static isRideable(typeId: string): boolean {
		return ["horse", "donkey", "mule", "camel", "pig", "strider"].some((p) =>
			typeId.includes(p),
		);
	}
